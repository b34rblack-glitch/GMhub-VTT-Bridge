...
| 0.6.0 | `v0.6.0` | **Agenda Editor hardening + entity-link picker (GMV-7).** Sequenced hardening → feature over `AgendaEditorDialog` and `agenda-editor.hbs`, no wire-format change. Hardening: a `pending` re-entrancy guard on Save (mirrors `VisibilityDialog`) so a double-click can't fire two overlapping `setFlag`/`update` sequences; up/down/remove row buttons gain localized `aria-label`s (icons `aria-hidden`) and 44px touch targets. Feature: a per-scene entity-link picker — an "Add entity" button opens a kind-grouped `<select>` of already-pulled entities and renders removable chips against the canonical `entities:[{id,name,entityType}]` shape (new exported `listPulledEntities()` in `sync.js`); the pinned row's brittle free-text `entity_id`/`entity_type` inputs are replaced by the same single-select picker (writing the snake `entity_id`/`entity_type`/`name` shape, with a "(not pulled)" fallback option preserving stored links). Both round-trip through Push/Pull and render as content-links. |
...
- **GMV-7** — AgendaEditor add/edit per-scene entity links. Closed in v0.6.0. An "Add entity" picker sources already-pulled entities (grouped by kind) into removable per-scene chips (`entities:[{id,name,entityType}]`), and the pinned row's free-text `entity_id`/`entity_type` inputs become the same single-select picker; both round-trip through Push/Pull and render as content-links. No wire-format change — Save already persisted the whole row object to structured flags.
